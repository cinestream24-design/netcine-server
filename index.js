...
// Resolve o stream de um canal: tenta m3u8 direto na página; se não
// achar, usa o mesmo mecanismo de navegador headless (Playwright) já
// usado no EmbedPlayer — funciona sem depender do formato do JS do site.
async function resolveCanalStream(canal) {
    const pageUrl = CANAIS_BASE + canal.slug;

    // 1. Tenta achar m3u8 direto no HTML da página
    try {
        const r    = await withTimeout(fetch(pageUrl, { headers: { 'User-Agent': UA } }), 7000);
        const html = await r.text();
        const m = html.match(/https?:\/\/[^\s"'<>]+\.m3u8[^\s"'<>]*/i);
        if (m) return { url: m[0], referer: pageUrl };
    } catch (e) {
    ...
    const master = await withTimeout(resolveViaPlaywright(pageUrl), 18000).catch(() => null);
    if (master) return { url: master, referer: pageUrl };
