...
async function embedGetUrl(imdbId) {
    try {
        ...
        if (!Object.keys(ids).length) { console.log('[Embed] Nenhum idS'); return []; }
        const streams = [];
        for (const [lang, idS] of Object.entries(ids)) {
            try {
                const apiRes = await fetch(EMBED_API, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/x-www-form-urlencoded', 'User-Agent': UA, 'Referer': embedUrl, 'Origin': 'https://embed.embedplayer.site' },
