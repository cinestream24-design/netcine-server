...
async function getTmdbInfo(imdbId, tipo) {
    try {
        ...
        try {
            ...
            const generos = (det.genres||[]).map(g => g.name.toLowerCase());
...
async function buscar(host, titulo, ano, isSerie, info = {}) {
    ...
    while ((m = blocoRe.exec(html)) !== null) {
        ...
        if (!similar(tituloSite, titulo)) { console.log('[NetCine] Rejeitado:', tituloSite, 'vs', titulo); continue; }

        let score = 0;
        if (ano && anoSite) score += (Math.abs(anoSite - parseInt(ano)) === 0) ? 10 : 5;
        if (norm(tituloSite) === norm(titulo)) score += 8;

        // Diferencia animacao de live action
        const tl = tituloSite.toLowerCase();
        if (info.animacao === true && (tl.includes('anima') || tl.includes('desenho'))) score += 5;
        if (info.animacao === false && (tl.includes('anima') || tl.includes('desenho'))) score -= 15;

        const pageUrl = new URL(href, host).href;
        candidatos.push({ pageUrl, tituloSite, anoSite, score });
        console.log('[NetCine] Candidato:', tituloSite, anoSite, 'score:', score);
    }

    if (!candidatos.length) { console.log('[NetCine] Nao encontrado:', titulo); return null; }
    candidatos.sort((a, b) => b.score - a.score);
    const melhor = candidatos[0];
    console.log('[NetCine] Escolhido:', melhor.tituloSite, melhor.anoSite);
...
async function embedGetUrl(imdbId) {
    try {
        ...
        const ids = {};
        let m;
        while ((m = itemRe.exec(html)) !== null) ids[m[1]] = m[2];

        if (!Object.keys(ids).length) { console.log('[Embed] Nenhum idS encontrado'); return []; }
        ...
        for (const [lang, idS] of Object.entries(ids)) {
            try {
                ...
                streams.push({ lang, url, label: lang === 'dub' ? '🇧🇷 [EP] Dublado' : '🎬 [EP] Legendado' });
            } catch(e) { console.log('[Embed] Erro lang', lang, ':', e.message); }
        }
        return streams;
    } catch(e) { console.log('[Embed] Erro:', e.message); return []; }
}
    ...
    try {
        ...
        async function buscarNetCine() {
            ...
            for (const p of players) {
                ...
                out.push({ name: 'NetCine', title: p.lang === 'DUBLADO' ? '🇧🇷 [NC] Dublado' : '🎬 [NC] Legendado', url: streamUrl, behaviorHints: { notWebReady: false } });
            }
            return out;
        }

        async function buscarEmbed() {
            const embedStreams = await embedGetUrl(imdbId);
            const out = [];
            for (const s of embedStreams) {
                ...
                try {
                    ...
                    if (body.trim().startsWith('#EXTM3U')) {
                        ...
                        streamUrl = `${selfBase}/proxy/playlist?key=${key}`;
                    } else { streamUrl = finalUrl; }
                } catch(_) {}
                out.push({ name: 'EmbedPlayer', title: s.label, url: streamUrl, behaviorHints: { notWebReady: false } });
            }
            return out;
        }
        ...
        const streams = [
            ...
            ...(epResult.status === 'fulfilled' ? epResult.value : [])
        ];
        if (ncResult.status === 'rejected') console.log('[NetCine] Erro:', ncResult.reason?.message);
        if (epResult.status === 'rejected') console.log('[Embed] Erro:', epResult.reason?.message);
