...
// Cache temporário de m3u8 (para casos onde o rawM3u8 está disponível)
const _m3u8Cache = {};
...
async function resolvePlayer(iframeUrl) {
    ...
    try {
        ...
        // Extrai URL do .m3u8 / .php?token= do HTML do player
        let m3u8 = null;
        ...
        // 3. Qualquer <source src="...php?token=...">
        if (!m3u8) {
            const tokM = html.match(/<source[^>]+src="([^"]+token=[^"]+)"/i);
            if (tokM) m3u8 = tokM[1];
        }

        // 4. Fallback: segue redirect do hls.php para pegar URL com token
        if (!m3u8 && qs.n && qs.p) {
            const fbUrl = `${origin}/media-player/dist/playerhls.php?n=${qs.n}&p=${qs.p}`;
            console.log('[NetCine] Fallback redirect:', fbUrl.slice(0,70));
            const r = await fetch(fbUrl, {
                headers: hlsHeaders, redirect: 'follow', agent: proxyAgent
            });
            const finalUrl = r.url;
            const body = await r.text();
            if (body.trim().startsWith('#EXTM3U')) {
                // Já é o m3u8 direto
                console.log('[NetCine] rawM3u8 primeiras linhas:', body.split('\n').slice(0,5).join(' | '));
                return { url: finalUrl, headers: hlsHeaders, rawM3u8: body };
            }
            if (finalUrl !== fbUrl) m3u8 = finalUrl; // seguiu redirect para URL com token
        }

        if (m3u8) {
            if (m3u8.startsWith('//')) m3u8 = 'https:' + m3u8;
            else if (!m3u8.startsWith('http')) m3u8 = new URL(m3u8, fetchUrl).href;
            // Segue redirect para obter URL final com token
            try {
                ...
                if (body2.trim().startsWith('#EXTM3U')) {
                        console.log('[NetCine] rawM3u8 primeiras linhas:', body2.split('\n').slice(0,5).join(' | '));
            return { url: finalUrl, headers: hlsHeaders, rawM3u8: body2 };
                }
                return { url: finalUrl, headers: hlsHeaders };
            } catch(e2) {
                console.log('[NetCine] Erro seguindo redirect:', e2.message);
                return { url: m3u8, headers: hlsHeaders };
            }
        }

        console.log('[NetCine] Player não resolvido. Snippet:', html.slice(0,300));
...
app.get('/proxy/playlist', async (req, res) => {
    const { url, hdrs, key } = req.query;

    // Serve m3u8 do cache em memória (rawM3u8)
    if (key && _m3u8Cache[key]) {
        const cached   = _m3u8Cache[key];
        const base     = cached.base.substring(0, cached.base.lastIndexOf('/') + 1);
        const selfBase = req.protocol + '://' + req.get('host');
        console.log('[NetCine] /proxy/playlist cache base:', base);
        let m3u8 = cached.text.split('\n').map(line => {
            const t = line.trim();
            if (!t || t.startsWith('#')) return line;
            const absUrl = t.startsWith('http') ? t : base + t;
            console.log('[NetCine] seg reescrito:', absUrl.slice(0,80));
...
app.get('/proxy/seg', async (req, res) => {
    ...
    console.log('[NetCine] /proxy/seg:', decodedUrl.slice(0, 80));
    try {
        const r = await fetch(decodedUrl, {
            headers: { 'User-Agent': UA, Referer: 'https://eee1.lat/' },
            redirect: 'follow',
            agent: proxyAgent
        });
        if (!r.ok) {
            console.log('[NetCine] Seg status:', r.status, decodedUrl.slice(0,60));
...
app.get('/stream/:type/:id.json', async (req, res) => {
    ...
    try {
        ...
        const results = await Promise.all(players.map(async p => {
            ...
            // Se temos o m3u8 bruto, servimos via proxy (caso raro)
            // Caso contrário entregamos a URL final com token direto ao Nuvio
            if (resolved.rawM3u8) {
                // rawM3u8: já temos o conteúdo, reescreve e serve via proxy público
                const key = Buffer.from(streamUrl).toString('base64').slice(0,32);
                _m3u8Cache[key] = { text: resolved.rawM3u8, headers: resolved.headers, base: streamUrl };
                // Usa host público (Railway) — não localhost
                const selfBase = req.protocol + '://' + req.get('host');
