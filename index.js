...
app.get('/proxy/playlist', async (req, res) => {
    ...
    if (key && _m3u8Cache.has(key)) {
        ...
        const origin = new URL(cached.base).origin;
        const segUrls = [];
        ...
        const m3u8 = cached.text.split('\n').map(line => {
            ...
            let absUrl;
            if (t.startsWith('http')) absUrl = t;
            else if (t.startsWith('/')) absUrl = origin + t;   // root-relative (ex: /m3/xxx)
            else absUrl = base + t;                             // relativo à pasta do master
            segUrls.push(absUrl);
