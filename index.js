...
async function resolvePlayer(iframeUrl) {
    ...
    try {
        ...
        if (m) {
            // Retorna o hls.php em vez da URL com token (que expira)
            // O Nuvio seguirá o redirect e pegará o token fresco
            console.log('[NetCine] HLS via source:', m[1].slice(0,60));
            return { url: fetchUrl, headers };
        }

        const all = [...html.matchAll(/<source[^>]+src="([^"]+)"/gi)];
        if (all.length) {
            console.log('[NetCine] HLS via source generico');
            return { url: fetchUrl, headers };
        }
